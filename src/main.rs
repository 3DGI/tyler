    ...
    if !cli.cesium3dtiles_tileset_only {
        ...
        let tiles_failed_iter = tiles.into_par_iter().map(|(tile, tileid)| {
            ...
            if format == Formats::_3DTiles {
                cmd = update_command_3dtiles(&cli, &world, cmd);
            }
...
/// Update a command with 3D Tiles specific arguments.
fn update_command_3dtiles(cli: &cli::Cli, world: &parser::World, mut cmd: Exec) -> Exec {
    // geof specific args
